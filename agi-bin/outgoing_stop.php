...
<?php
/**
 * Update channel variables to correctly reflect call in CDR
 */
...
$select = 'SELECT id FROM agents WHERE interface="' . $interface . '"';
...
execute_agi('GET VARIABLE CDR(duration)');

errlog('Extention dialed is ' . $agi['dnid']);

$duration = execute_agi("GET VARIABLE CDR(duration)");


$query = 'INSERT INTO queue_log (time, callid, queuename, agent, event, data) VALUES(now(), ' . $agi['uniqueid'] . ', "outgoing", "' . $interface . '", "COMPLETEAGENT", "|' . $agi['dnid'] . '")';
